...
export function registerRoutes({ app, db, idService, logger, chunkingService, embeddingService, authMiddleware }: RouteDependencies): void {
  // Feature modules
  ...
  const sourcesRoutes = new SourcesRoutes(db, idService, logger, chunkingService, embeddingService, authMiddleware);
